...
His résumé summarises him as a backend developer with two years of experience designing APIs and data models for production web applications in Node.js and Django, who has replaced manual workflows with automation, cut database load and client-side latency on live systems, and built and operated a multi-tenant SaaS product solo including its AWS infrastructure and CI/CD. It notes he is currently building AI-driven features into production.`,
  ...
  gaps: [
    ...
    "Service meshes and large-scale distributed systems — outside his experience. PropVexis runs on a single instance, not a fleet: three isolated environments share one t3.micro and one native PostgreSQL, and it ships one Node worker per environment. If asked about the 1,000-concurrent-users figure, be precise about what it is: a standing design bar he holds the code to, with tests that fail if someone reintroduces a per-request query in the auth path — not measured production traffic. He has not load-tested to that number and should not be described as serving it.",
    "Horizontal scaling of PropVexis — the correctness work is done but not switched on. The shared Socket.IO adapter and cross-worker cache invalidation are written and tested behind a REDIS_URL flag, so Redis is a code path rather than a running dependency; the blockers to raising the worker count are provisioning Redis, an instance upsize, and lowering the per-process connection pool. Do not describe PropVexis as running clustered or as running Redis in production.",
